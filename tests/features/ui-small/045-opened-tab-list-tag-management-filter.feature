...
Feature: Opened Tab List - Tag Management Filter
    ...
    Background:
        ...
        Then I should not see that no tag matches tag search on the tab tag management view

    Scenario: Should filter tab tags by label on input with one word
        When I type "azerty" in the tag filter input on the tab tag management view

        Then I should see 1 visible tag in the tab tag management view
        And I should see the tag 0 with the label "the azerty label" in the tab tag management view
        And I should not see that no tag matches tag search on the tab tag management view

    Scenario: Should filter tab tags by label on input with two word
        When I type "azerty qwerty" in the tag filter input on the tab tag management view

        Then I should see 2 visible tags in the tab tag management view
        And I should see the tag 0 with the label "the azerty label" in the tab tag management view
        And I should see the tag 1 with the label "the qwerty label" in the tab tag management view
        And I should not see that no tag matches tag search on the tab tag management view

    Scenario: Should show the no tag row in tab tags list when the filter do not match any tag
        When I type "unknown" in the tag filter input on the tab tag management view

        Then I should see 0 visible tag in the tab tag management view
        And I should see that no tag matches tag search on the tab tag management view

        When I delete all characters in the tag filter input on the tab tag management view
    ...
    Scenario: Should show all tab tags when clearing the input
        ...
        Then I should see 4 visible tags in the tab tag management view
        And I should see the tag 0 with the label "label one" in the tab tag management view
        And I should see the tag 1 with the label "label two" in the tab tag management view
        And I should see the tag 2 with the label "the azerty label" in the tab tag management view
        And I should see the tag 3 with the label "the qwerty label" in the tab tag management view
        And I should not see that no tag matches tag search on the tab tag management view
    ...
    Scenario: Should apply filter on new tags
        ...
        Then I should see 2 visible tags in the tab tag management view
        And I should see the tag 0 with the label "matching azerty" in the tab tag management view
        And I should see the tag 1 with the label "the azerty label" in the tab tag management view
